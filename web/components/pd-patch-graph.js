...
function nodeLabel(node) {
  ...
  switch (k.kind) {
    case 'obj':
      return [k.name, k.args?.map((a) => a.value ?? '').join(' ')].filter(Boolean).join(' ')
    case 'msg':
      return (
        k.messages
          ?.flat()
...
class PdPatchGraph extends HTMLElement {
  ...
  get title() {
    return this._title || this.getAttribute('title') || 'patch'
  }
  ...
  _drawGraph() {
    ...
    for (const conn of conns) {
      ...
      const d = dy > 0 ? `M${x1},${y1} C${x1},${y1 + bend} ${x2},${y2 - bend} ${x2},${y2}` : `M${x1},${y1} C${x1},${y1 + bend} ${x2},${y2 + bend} ${x2},${y2}`
