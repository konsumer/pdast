...
Not yet implemented. The plan (from `PLAN.md`) is:
- `wasm_parse_to_json(content: &str) -> String` — always available, works in WASI/non-JS hosts.
- `#[wasm_bindgen] fn wasm_parse(content: &str) -> JsValue` — behind a `wasm-js` feature flag, returns a JS object directly.

To add: create `pdast/src/wasm.rs`, add `wasm-bindgen` and `serde-wasm-bindgen` as optional dependencies in `pdast/Cargo.toml` under `[target.'cfg(target_arch = "wasm32")'.dependencies]`, and expose `parse_patch_no_loader` through the WASM bindings.
